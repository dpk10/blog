...
const CreateBlog = () => {
  ...
  const handleBlogSubmit = async (e) => {
    ...
    const blogPayload = {
      ...
      tags: blogData.tags.split(',').map(tag => tag.trim()), // Ensure tags are split properly
      subcategory: blogData.subcategory.split(',').map(sub => sub.trim()), // Ensure subcategories are split properly
    };

    try {
      const result = await createBlog(blogPayload, token); // Await the createBlog call to handle the async operation properly
  ...
  return (
    <div className="flex justify-center items-center h-screen"
      ...
      <ToastContainer />
      <div className="w-full max-w-lg shadow-lg rounded-lg p-6 border-x-10 opacity-75">
